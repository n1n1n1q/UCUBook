...
function createPaginationArrows(
  numPages,
  currentPage,
  container,
  requests,
  ...
  prevArrow.addEventListener("click", () =>
    showPage(currentPage - 1, requests, container, requestType)
  );
  if (currentPage === 1) {
  ...
  currentRequests.forEach((request) => {
    ...
    if (requestType === "pending") {
      ...
      acceptButton.addEventListener("click", () =>
        updateRequestStatus(request, 1, requestDiv)
      );
      ...
      declineButton.addEventListener("click", () =>
        updateRequestStatus(request, 2, requestDiv)
      );
...
filterButton.addEventListener("click", function () {
  ...
  const prevRequests = prevContainer.querySelectorAll(
    ".prev-requests, hr, span"
  ); // Select all request elements
  ...
  fetch("/get_past_requests")
    ...
    .then((pastRequests) => {
      ...
      filteredRequests = filter_by_date(
        selectedDate,
        filter_by_room(selectedRoom, pastRequests)
      );
